...
class FeatureFlagManager:
    ...
    def _init_unleash(self):
        ...
        try:
            ...
            return UnleashClient(url=url, app_name="spx-trading-bot", custom_headers={'Authorization': api_key})
        except ImportError:
    ...
    def gradual_rollout(self, feature_name: str, target_percentage: float = 100.0, step_size: float = 10.0, step_delay: int = 300) -> bool:
        """Gradually roll out a feature"""
        logger.info(f"Starting gradual rollout of '{feature_name}' to {target_percentage}%")
        ...
        while current_percentage < target_percentage:
            ...
            if not self.enable_feature(feature_name, next_percentage):
                logger.error(f"Failed to update feature '{feature_name}' to {next_percentage}%")
                return False
        ...
        logger.info(f"Gradual rollout of '{feature_name}' completed at {target_percentage}%")
        return True
    ...
    def _update_launchdarkly_flag(self, flag_key: str, enabled: bool, percentage: float) -> bool:
        """Update LaunchDarkly flag via REST API"""
        # This would require the LaunchDarkly Management API
        # Implementation would depend on specific requirements
        logger.info(f"Would update LaunchDarkly flag {flag_key}: {enabled} at {percentage}%")
        return True
...
Examples:
  ...
  python scripts/update_feature_flags.py --list-flags
    ...
    parser.add_argument(
        '--deployment-id',
        help='Deployment ID for tracking'
    )
    ...
    parser.add_argument(
        '--rollout',
        help='Gradually roll out a feature'
    )
    ...
    parser.add_argument(
        '--rollback',
        action='store_true',
        help='Rollback features to safe state'
    )
    ...
    try:
        ...
        if args.health_check:
            ...
            if args.deployment_id:
                logger.info(f"Applying default feature flags for deployment {args.deployment_id}")
