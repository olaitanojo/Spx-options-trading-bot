...
class SPXOptionsBot:
    ...
    def calculate_position_size(self, strategy: TradingStrategy, option_price: float) -> int:
    ...
    def backtest_strategy(self, start_date: str, end_date: str) -> Dict:
        ...
        mask = (data_with_indicators.index >= start_date) & (data_with_indicators.index <= end_date)
        ...
        win_rate = len([pos for pos in backtest_positions if pos.pnl > 0]) / max(total_trades, 1)
        ...
        logger.info(f"Backtest completed: {total_trades} trades, {win_rate:.2%} win rate, {results['return_pct']:.2f}% return")
        return results
    ...
    def run_live_analysis(self) -> Dict:
        ...
        analysis = {
            ...
            'price_vs_sma20': ((current_data['Close'] - current_data['SMA_20']) / current_data['SMA_20']) * 100,
            'recommendation': self._generate_recommendation(current_data, current_signal)
        }
        
        logger.info(f"Current signal: {analysis['signal']}, Price: ${analysis['current_price']:.2f}")
        return analysis
    
    def _generate_recommendation(self, current_data: pd.Series, signal: TradeSignal) -> str:
        ...
        if signal == TradeSignal.BUY:
            ...
            if current_data['RSI'] < 40:
                recommendations.append("RSI indicates oversold conditions - good for calls")
        elif signal == TradeSignal.SELL:
            recommendations.append("Consider buying put options")
            if current_data['RSI'] > 60:
                recommendations.append("RSI indicates overbought conditions - good for puts")
        else:
            recommendations.append("No clear signal - consider staying in cash")
        
        vix_level = current_data.get('VIX', 0)
        if vix_level > 25:
            recommendations.append("High VIX suggests elevated volatility - consider shorter-term trades")
        elif vix_level < 15:
            recommendations.append("Low VIX suggests low volatility - consider longer-term trades")
